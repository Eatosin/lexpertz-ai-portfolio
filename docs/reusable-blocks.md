...
| **SocialCloud**                          | `ui/social-cloud.tsx`                      | `githubUrl?, twitterUrl?, linkedinUrl?, facebookUrl?, emailUrl?, className` — circular 44px social chips, color-agnostic (`text-current`), missing links filtered                              | Footer brand card                                     | Author/company social rows, any profile surface              |
